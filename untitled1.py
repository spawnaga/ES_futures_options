...
class all_events():
    ...
    def show_error(self, reqId, errorCode, errorString, contract):
...
def trade(ES, hasNewBar=None, p=-1):
    ...
    print(
        f'cash in hand = {cash_in_hand}, portfolio value = {portolio_value}, unrealized PNL = {account[32].value}, realized PNL = {account[33].value}, holding = {stock_owned[0]} calls and {stock_owned[1]} puts and ES = {data_raw.iloc[-1, 3]} and [call,puts] values are = {options_array}')
    if df["high"].iloc[-1] >= df["roll_max_cp"].iloc[-2] and \
            df["volume"].iloc[-1] > df["roll_max_vol"].iloc[-2] and \
            len(ib.portfolio()) == 0 and len(ib.reqAllOpenOrders()) == 0 and buy_index == []:
        ...
        while can_buy:
            ...
            for i in buy_index:
                ...
                if cash_in_hand > (options_array[i] * 50) and cash_in_hand > portolio_value \
                        and ((stock_owned[0] == 0 and i == 0) or (stock_owned[1] == 0 and i == 1)) and len(
...
if __name__ == "__main__":
    ...
    try:
        ...
        call_option_volume = roll_contract(call_option_volume, call_option_price.bidSize)
        put_option_volume = roll_contract(put_option_volume, put_option_price.bidSize)
        account = events.account_update(0)
        ib.accountSummaryEvent += events.account_update
