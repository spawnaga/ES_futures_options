...
class get_data:
    ...
    def ES(self, ES, ATR):
        ...
        ES_df['RSI'] = ta.RSI(ES_df['close'], timeperiod=21)
...
class Trade:
    ...
    def __init__(self):
        ...
        self.call_option_price_average = np.ones(2)  # start call options volume array to get the max volume in the last 20
        self.call_option_price_average = np.ones(2)
    ...
    def strategy(self, df):
        ...
        if self.stock_owned[0] == 0 and self.stock_owned[1] == 0 and df["bar_num"].iloc[i - 1] >= 2 and \
            ...
            return buy_index, sell_index, take_profit
                ...
                df["obv_slope"].iloc[i - 1] < -25 and (not (df["close"].iloc[i-1] <= df["roll_min_cp"].iloc[i - 2] + 0.5)) and\
            ...
            return buy_index, sell_index, take_profit
                ...
                (self.call_option_price_average[0] - self.call_cost) <= -1 * stop_loss) and (
                                                     self.call_option_price_average[
                                                         1] - self.call_cost) <= -1 * stop_loss) <= -1 * stop_loss) and self.submitted == 0:
            ...
            return buy_index, sell_index, take_profit
                ...
                (self.put_option_price_average[0] - self.put_cost) <= -1 * stop_loss) and (self.put_option_price_average[1] - self.put_cost) <= -1 * stop_loss)) \
