...
def _get_obs(stock_owned, stock_price, cash_in_hand):
    obs = np.empty(5+len(data.iloc[-1,:-2]))
    obs[:2] = stock_owned
...
def trade(ES, hasNewBar=None):
    ...
    data_raw = res.options(res.options(res.ES(ES),res.option_history(res.get_contract('C', 2000))),res.option_history(res.get_contract('P', 2000)))
    ...
    if data_raw["close"].iloc[-1] > data_raw["close"].iloc[-2] - (0.75 * data_raw["ATR"].iloc[-2]) and len(ib.positions())!=0 and sell_index==[]:
        sell_index.append(0)

    
    elif data_raw["close"].iloc[-1] < data_raw["close"].iloc[-2] + (0.75 * data_raw["ATR"].iloc[-2]) and len(ib.positions())!=0 and sell_index==[]:
        sell_index.append(1)
