...
""" Note: this code uses trading strategies and some functions coned from this course in udemy :
(Algorithmic Trading & Quantitative Analysis Using Python) by the great instructor Mayank Rasu


The functions(Slope and Renko_df) were clonned from the mentioned course.
...
class get_data:
    ...
    def ES(self, ES, ATR):
        ...
        ES_df['ATR'] = ta.ATR(ES_df['high'], ES_df['low'], ES_df['close'], timeperiod=10)
        ...
        ES_df = renko_df(ES_df, ATR)  # Get RENKO chart and obv slope
        return ES_df
...
class Trade:
    ...
    def __init__(self):
        ...
        self.portfolio = []
        self.connect()
        ES = Future(symbol='ES', lastTradeDateOrContractMonth='20210319', exchange='GLOBEX', currency='USD')  # define
        ...
        # self.ES_streaming = ib.reqMktData(ES)
    ...
    def trade(self, ES, hasNewBar=None):
        ...
        if self.data_raw.iloc[-1, 1] == 0:
            return
        df = self.data_raw[
            ['date', 'high', 'low', 'close', 'volume', 'bar_num', 'obv_slope', 'macd', 'macdsignal', 'macd_slope', 'macd_sig_slope', 'ATR', 'RSI', 'EMA_9-EMA_26',
             'roll_max_cp',
             'roll_min_cp', 'B_upper', 'B_middle', 'B_lower', 'roll_max_vol']]# filter data
        ...
        if self.stock_owned[0] > 0:
            ...
            print(self.call_option_price.contract)
            ...
            print(self.put_option_price.contract)
    ...
    def strategy(self, df):
        ...
        print(
            f'time = {self.data_raw.iloc[-1, 0] - timedelta(hours=12)} cash in hand = {self.cash_in_hand}, portfolio value = {self.portfolio_value}, unrealized PNL ='
            f' {self.unrealizedPNL} realized PNL = {self.realizedPNL}, holding = {self.stock_owned[0]} '
            f'calls and {self.stock_owned[1]} puts and ES = {self.data_raw.iloc[-1, 4]} and bar_num = {df["bar_num"].iloc[-1]} and obv_slope = {df["obv_slope"].iloc[-1]}'
            ...
            f' lower BBand = {df["B_lower"].iloc[i - 1]} and macd(macd, signal, macd_slope, macd_sig_slope) are ({df["macd"].iloc[-1], df["macdsignal"].iloc[-1], df["macd_slope"].iloc[-1], df["macd_sig_slope"].iloc[-1]} and '
        ...
        #         df['RSI'].iloc[i - 1] < 90 and buy_index == [] and self.submitted == 0:
        #     print("Buy call")
        #     buy_index.append(0)
        #     self.submitted = 1
        #     return buy_index, sell_index, take_profit
        ...
        #         ((df["bar_num"].iloc[i - 1] < -2 and
        #           df["obv_slope"].iloc[i - 1] < -25)) and \
        #         df['RSI'].iloc[i - 1] > 15 and buy_index == [] and self.submitted == 0:
        #     print("Buy put")
        #     buy_index.append(1)
        #     self.submitted = 1
        #     return buy_index, sell_index, take_profit
                ...
                ((df["bar_num"].iloc[i] >= 2 and
                  df["macd"].iloc[i] > df["macdsignal"].iloc[i] and \
                  df["macd_slope"].iloc[i] > df["macd_sig_slope"].iloc[i])) and \
                df['RSI'].iloc[i - 1] < 90 and df['obv_slope'].iloc[-1] >= 25 and buy_index == [] and self.submitted == 0:
            print("Buy call")
            buy_index.append(0)
            self.submitted = 1
            return buy_index, sell_index, take_profit
                ...
                ((df["bar_num"].iloc[i] <= -2 and
                  df["macd"].iloc[i] < df["macdsignal"].iloc[i] and \
                  df["macd_slope"].iloc[i] < df["macd_sig_slope"].iloc[i])) and \
                df['RSI'].iloc[i - 1] > 15 and df['obv_slope'].iloc[-1] <= -25 and buy_index == [] and self.submitted == 0:
            print("Buy put")
            buy_index.append(1)
            self.submitted = 1
            return buy_index, sell_index, take_profit
        ...
        #     self.submited = 1
        #     ib.sleep(5)
        #     if df["high"].iloc[i] >= df["roll_max_cp"].iloc[i - 4]:
        #         return buy_index, sell_index, take_profit
        #     else:
        #         self.submitted = 0
        #         return [], [], []
        ...
        #     self.submitted = 1
        #     ib.sleep(5)
        #     if df["low"].iloc[i] <= df["roll_min_cp"].iloc[i - 4]:
        #         return buy_index, sell_index, take_profit
        #     else:
        #         self.submitted = 0
        #         return [], [], []
        #
                ...
                and (df["low"].iloc[i] <= df["roll_min_cp"].iloc[i - 1] and
                     df["volume"].iloc[i] > df["roll_max_vol"].iloc[i - 1]) * 1.5 and \
            ...
            return buy_index, sell_index, take_profit
                ...
                (df["high"].iloc[i] >= df["roll_max_cp"].iloc[i - 1] and
                 df["volume"].iloc[i] > df["roll_max_vol"].iloc[i - 1]) * 1.5 \
            ...
            return buy_index, sell_index, take_profit
                ...
                ((((self.call_option_price.bid / self.call_cost) < 0.90) and (
                df["low"].iloc[i] <= df["close"].iloc[i - 1]) - df['ATR'].iloc[-1])) and \
            ...
            return buy_index, sell_index, take_profit
                ...
                ((((self.put_option_price.bid / self.put_cost) < 0.95) and (
                         df["high"].iloc[i] >= df["close"].iloc[i - 1]) + df['ATR'].iloc[-1])) and \
        ...
        elif (self.stock_owned[0].all() > 0) and (self.call_option_price.bid - self.call_cost >= 1) and \
                ((df['macd'].iloc[-1] < df['macdsignal'].iloc[i] and df['macd_slope'].iloc[i] < df['macd_sig_slope'].iloc[
                    i]) and (df["obv_slope"].iloc[i] < -25)) \
            ...
            return buy_index, sell_index, take_profit
                ...
                (self.put_option_price.bid - self.put_cost >= 1) and \
                ((df['macd'].iloc[-1] > df['macdsignal'].iloc[i] and df['macd_slope'].iloc[i] > df['macd_sig_slope'].iloc[i]) and \
                 (df["obv_slope"].iloc[i] > 15)) \
                and self.submitted == 0:
        ...
        elif self.barnumb_lock is True and (df["bar_num"].iloc[i] == 1 or df["bar_num"].iloc[i] == -1):
            self.submitted = 0
            self.barnumb_lock = False
            self.barnumb_value = 0
            return buy_index, sell_index, take_profit
    ...
    def open_position(self, contract, quantity, price):  # start position
        if len(ib.portfolio()) > 0 or len(ib.reqAllOpenOrders()) > 0:
            ...
            self.submitted = 0
            return
        trigger_time = self.data_raw.iloc[-1, 0]

        # while self.data_raw[-1,0] == trigger_time:
        #     print('wait for a confirmation')
        # if contract.right == 'C' and self.data_raw.iloc[-1, 4] < ES_price-0.25:
        #     print('cancel buying calls False breakout')
        #     return
        # elif contract.right == 'P' and self.data_raw.iloc[-1, 4] > ES_price+0.25:
        #     print('cancel buying puts False breakout')
        #     return
        ...
        quantity = 1 if (
                self.second_calls_buy is False or self.second_puts_buy is False) else 2  # quantity if quantity < 4 else 3
    ...
    def option_position(self, event=None):
        try:
            ...
            print("*************** error at ib.position() trying to reconnect ******")
        ...
        if len(position) == 0:
            ...
            ib.sleep(1)
            self.contract_time = datetime.now(timezone.utc) - timedelta(hours=1)
            return
        else:
